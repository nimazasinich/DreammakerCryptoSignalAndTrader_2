...
export const DashboardView: React.FC = () => {
  ...
  const [portfolio, setPortfolio] = useState<PortfolioSummary>({
    ...
    if (loading && portfolioValue === 0) {
        return (
            <ErrorBoundary>
                <div className="w-full min-h-full animate-fade-in p-6">
                    <DashboardSkeleton />
                </div>
            </ErrorBoundary>
        );
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="relative z-10 p-6">
                                {/* Icon */}
                                <div
                                    className="inline-flex p-3 rounded-xl mb-4 group-hover:scale-110 transition-transform duration-500 relative"
                                    style={{
                                ...
                                >
                                    ...
                                    <div 
                                        ...
                                        style={{
                                            background: `radial-gradient(circle, rgba(${stat.glowColor}, 0.4) 0%, transparent 70%)`
                                        }}
                                    />
                                </div>
                                ...
                                <div>
                                    <div className="flex items-center justify-between mb-2">
                                        <p className="text-slate-400 text-[10px] font-medium tracking-wide uppercase">{stat.label}</p>
                                        <div className="w-1.5 h-1.5 rounded-full bg-slate-500 opacity-50 group-hover:opacity-100 transition-opacity" />
                                    </div>
                                    <p className="text-2xl font-bold text-white mb-1 tracking-tight group-hover:scale-105 transition-transform duration-300" style={{
                                    ...
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className="text-[10px] text-slate-500 font-medium">
                                            {stat.label === 'Total Portfolio' && portfolioValue !== undefined 
                                                ? `24h: ${portfolioChange >= 0 ? '+' : ''}${portfolioChange.toFixed(2)}%`
                                                : stat.label === '24h P&L' && dayPnLPercent !== undefined
                                                ? `${dayPnLPercent >= 0 ? '+' : ''}${dayPnLPercent.toFixed(2)}%`
                                                : ''}
                                        </span>
                                    </div>
                                    <p className="text-[10px] text-slate-500 leading-relaxed">{stat.subValue}</p>
                                    {/* Additional detail line */}
                                    {stat.label === 'Active Positions' && (
                                        <p className="text-[9px] text-slate-600 mt-1.5 opacity-75">
                                            Avg size: ${portfolioValue && activePositions > 0 ? (portfolioValue / activePositions).toLocaleString('en-US', { maximumFractionDigits: 0 }) : '0'}
                                        </p>
                                    )}
