...
export async function fetchOHLCV(symbol, timeframe, limit = 500) {
...
export async function fetchUniverseTopN(n = 300) {
    ...
    const res = await fetch(url, { credentials: 'include' });
    if (!res.ok)
        throw new Error(`universe ${res.status}`);
    return res.json();
}
//# sourceMappingURL=RealDataProvider_from_LastChance.js.map